...
const convert = (path, row) => new Promise((resolve, reject) => {
    ...
    console.log('Processing %s and %s', row.thumbnail_filename, row.large_filename);

    const image_thumb = new Buffer(row.thumbnail, 'hex');
    const thumbnail_filename = row.thumbnail_filename.replace(/\.gif$/, '');
    gm(image_thumb, `${thumbnail_filename}.gif`)
        .write(`${path}/images/${thumbnail_filename}.png`, (err) => {
            if (err) {
                console.error(err);
            }
        });
    ...
    gm(image_large, `${large_filename}.gif`)
        .write(`${path}/images/${large_filename}.png`, (err) => {
            if (err) {
                console.error(err);
            }
        });
...
module.exports = function (path) {
    ...
    return Rx.Node
        .fromStream(fs.createReadStream(`${path}/ProductPhoto.csv`, { encoding: 'utf16le' })
            .pipe(csv.parse({
                delimiter: '|',
                rowDelimiter: '\r\n',
                columns: ['id', 'thumbnail', 'thumbnail_filename', 'large', 'large_filename', 'date', 'ignore']
            })))
